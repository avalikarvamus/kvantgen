...
function loadStar(id) {
	...
	success: function(json) {
		$('#sidebar').html("");
		console.log("outside star");
		console.log(json);
		$('#sidebar').append("<br>Star: "+json.star.name+"<br>Coords: "+json.star.cx+":"+json.star.cy+"<br>Mass: "+json.star.mass);
		$(json).find('planets').find('planet').each(function(){
...
function loadMap() {
	...
	dataType: "json",
	success: function(json) {
		$(json).find('system').each(function(){
			var star = $(this).find('star');
			var id = $(star).find('id').text();
			var name = $(star).find('name').text();
			var cx = $(star).find('cx').text();
